...
function install_operator() {
  ...
  TAG=localhost/minio/operator:dev
  ...
  SIDECAR_TAG=localhost/minio/operator-sidecar:dev
  ...
  if [ "$1" = "helm" ]; then
    ...
    yq -i '.operator.image.repository = "localhost/minio/operator"' "${SCRIPT_DIR}/../helm/operator/values.yaml"
